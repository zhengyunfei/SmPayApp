...
<#-- 显示当前页号和它附近的页号 -->
		...
		<#if (pageNo==i)>
		<#if pageNo gt 1>
            <a href="javascript:goPage('${pageNo-1}')" onclick="turnOverPage(${pageNo-1});"  class="fetch_page pre" >上一页</a>
		  </#if>
			<#if (pageCount>pageNo)>
                <a href="javascript:goPage('${pageNo+1}')" onclick="turnOverPage(${pageNo+1});"  class="fetch_page next" >下一页</a>
			</#if>
		</#if>
		</#list>
	</div>
